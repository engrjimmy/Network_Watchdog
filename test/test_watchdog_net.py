...
possible_paths = [
    os.path.join(parent_dir, "watchdog_net.py"),
    os.path.join(parent_dir, "watchdog_net", "watchdog_net.py")
]
...
if not watchdog_file:
    print("ERROR: Could not find watchdog_net.py in any of these locations:")
    for path in possible_paths:
        print(f"  - {path}")
    sys.exit(1)
...
sys.modules["watchdog_net_module"] = watchdog_net  # This is important for patching to work
